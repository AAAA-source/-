...
nb_gross_margin = MultinomialNB()
nb_gross_margin.fit(X_train, y_train)

# 預測並評估毛利率標籤模型
y_pred_gross_margin = nb_gross_margin.predict(X_test)
print("毛利率標籤預測報告：")
print(classification_report(y_test, y_pred_gross_margin))

# 輸出混淆矩陣
conf_matrix_gross_margin = confusion_matrix(y_test, y_pred_gross_margin)
print("毛利率標籤混淆矩陣：")
print("       0.   1.   2.")
for i, row in enumerate(conf_matrix_gross_margin):
    print(f"{i}. {row[0]:>5} {row[1]:>5} {row[2]:>5}")

# 訓練ROA標籤預測模型
X_train, X_test, y_train, y_test = train_test_split(X, roa_column, test_size=0.2, random_state=42)
nb_roa = MultinomialNB()
nb_roa.fit(X_train, y_train)

# 預測並評估ROA標籤模型
y_pred_roa = nb_roa.predict(X_test)
print("ROA標籤預測報告：")
...
print("ROA標籤混淆矩陣：")
print("       0.   1.   2.")
for i, row in enumerate(conf_matrix_roa):
    print(f"{i}. {row[0]:>5} {row[1]:>5} {row[2]:>5}")
